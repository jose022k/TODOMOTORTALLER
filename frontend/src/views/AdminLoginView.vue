<template>
  <div class="auth-container">
    <div class="auth-card">
      <h1 class="auth-title">Inicio de Sesión - Administrador</h1>
      <form @submit.prevent="handleLogin">
        <div class="form-group">
          <label for="email">Email</label>
          <input id="email" v-model="email" type="email" placeholder="admin@email.com" required />
        </div>
        <div class="form-group">
  ...
  methods: {
    async handleLogin() {
      ...
      try {
        ...
        await authStore.login(this.email, this.password, "admin");
