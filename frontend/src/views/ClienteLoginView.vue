<template>
  <div class="auth-container">
    <div class="auth-card">
      <h1 class="auth-title">Inicio de Sesión - Cliente</h1>
      ...
      <div class="auth-link">
        ¿No tienes cuenta? <router-link to="/register/cliente">Regístrate aquí</router-link>
      </div>
    </div>
  </div>
</template>
  ...
  methods: {
    async handleLogin() {
      ...
      try {
        ...
        await authStore.login(this.email, this.password, "cliente");
        this.$router.push("/");
      } catch (err) {
